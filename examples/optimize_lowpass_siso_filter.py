...
from ltitop.models.transforms import discretize, lowpass_to_lowpass
from ltitop.topology.diagram.analysis import (
    analytic_diagram,
    error_bounds,
    output_range,
    signal_processing_function,
    spectral_radii,
)
...
class Criteria:
    ...
    number_of_adders: int = sys.maxsize
    number_of_multipliers: int = sys.maxsize
    size_of_memory: int = sys.maxsize
...
def evaluate(
    ...
    *,
    ...
    if signal_power != 0.0:
        ...
        criteria.arithmetic_snr = (
            10.0 * np.log10(float(signal_power / arithmetic_noise_power)).item()
        )  # force it to be an scalar
        ...
        response = 10 * np.log10(
            output_noise_power_density / input_noise_power_density + 1 / inf
        )  # ensure nonzero values
        criteria.frequency_response_error = np.sqrt(
            np.mean((expected_response - response) ** 2)
        )
...
def main():
    ...
    psd = functools.partial(signal.welch, scaling="density", window=window, fs=fs)
    ...
    if not only_visualize:
        with multiprocessing.Pool() as pool:
            try:
                ...
                stats = deap.tools.Statistics(key=lambda code: code.fitness.values)
    ...
    def fit_most_within_unit_interval(values):
        ...
        if not mad:
            return values
        return (values - med) / (4.0 * mad)

    plt.plot(
        gen, fit_most_within_unit_interval(crt.overflow_margin), label="Med[$M_o$]"
    )
    plt.plot(
        gen, fit_most_within_unit_interval(crt.underflow_margin), label="Med[$M_u$]"
    )
    plt.plot(
        gen, fit_most_within_unit_interval(crt.stability_margin), label="Med[$M_s$]"
    )
    plt.plot(
        gen,
    ...
    plt.plot(
        gen, fit_most_within_unit_interval(crt.number_of_adders), label="Med[$N_a$]"
    )
    plt.plot(
        gen,
    ...
    idx = solvers.gp.argnondominated(
        -frequency_response_error, arithmetic_snr, -memory_size_in_bytes
    )
    scatter = ax.scatter(
    ...
    output_noise = func(input_noise).T[0].astype(float)
    _, output_noise_power_density = psd(output_noise)
    optimized_implementation_a_response = 10 * np.log10(
        output_noise_power_density / input_noise_power_density + 1 / inf
    )
    ...
    output_noise = func(input_noise).T[0].astype(float)
    _, output_noise_power_density = psd(output_noise)
    optimized_implementation_b_response = 10 * np.log10(
        output_noise_power_density / input_noise_power_density + 1 / inf
    )
    ...
    if show_biquad_cascade:
        ...
        output_noise = func(input_noise).T[0].astype(float)
        _, output_noise_power_density = psd(output_noise)
        biquad_cascade_response = 10 * np.log10(
            output_noise_power_density / input_noise_power_density + 1 / inf
        )
...
if __name__ == "__main__":
    with FixedFormatArithmeticLogicUnit(
        format_=Q(15), allows_overflow=True, overflow_behavior=wraparound
    ):
        main()
