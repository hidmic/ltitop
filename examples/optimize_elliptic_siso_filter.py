...
def main():
    ...
    freq, output_noise_power_density = signal.welch(
        output_noise, fs, window=window, scaling='density')
    ...
    if not only_visualize:
        stats = deap.tools.Statistics(
            key=lambda code: code.fitness.values[4])
    ...
    ax.set_xlabel('Frequency Response Error [dB]')
    ax.set_ylabel('Stability Margin (log)')
    ax.invert_xaxis()
    ...
    ax.set_xlabel('Frequency Response Error [dB]')
    ax.set_ylabel('Arithmetic SNR [dB]')
    ax.invert_xaxis()
    ...
    fig.colorbar(scatter, ax=ax, label='Stability margin')
    ax.set_xlabel('Frequency Response Error [dB]')
    ax.set_ylabel('Arithmetic SNR [dB]')
    ax.set_zlabel('Memory size [bytes]')
    ax.invert_xaxis()
    ax.invert_zaxis()

    index = np.argsort(frequency_response_error)[2]
    implement = toolbox.compile(codes[index])
    optimized_implementation = implement(prototype)

    pretty(optimized_implementation).draw('optimized.svg')
    ...
    if show_biquad_cascade:
        ...
        # pretty(biquad_cascade).draw('result.svg')
    ...
    plt.plot(freq, optimized_implementation_response, label='Optimized')
