...
from ltitop.models.composites import parallel_composition, series_composition
...
from ltitop.topology.diagram.construction import parallel_diagram, series_diagram
...
def test_diagram_output_range():
    ...
    input_mean = 0.5
    input_delta = 0.5
    input_range = interval(input_mean - input_delta, input_mean + input_delta)
    dcg = (4.0 + 1.0) / 0.9
    ...
    assert (
        output_range(analytical_diagram, input_range, source=y, target=z)
        == input_range / 0.9
    )
...
def test_diagram_error_bounds():
    ...
    with FixedFormatArithmeticLogicUnit(
        ...
        assert error_bounds(analytical_diagram, source=y) in error_bounds(
            analytical_diagram
        )
...
def test_diagram_signal_processing():
    ...
    diagram = series_diagram(
        [parallel_diagram([F, G], output=y), H], input_=x, output=z
    )
    model = series_composition([parallel_composition([F.model, G.model]), H.model])
    _, (impulse_response,) = model.impulse(n=100)

    f = signal_processing_function(diagram)
    assert_allclose(f([1.0] + [0.0] * 99), impulse_response)
