...
def test_tempvar():
    var0, var1, var2, var3 = sympy.symbols("var0 var1 var2 var3")
...
def test_block_as_diagram():
    block = DirectFormI.from_model(signal.dlti(1, [1, 1, 0.5]))
    x, y = sympy.symbols("x y")
    diagram = as_diagram(block, input_=x, output=y)
    assert len(diagram[x][y]) == 1
    assert diagram[x][y][0]["block"] is block


def test_series_diagram_construction():
    head_block = DirectFormI.from_model(signal.dlti(1, [1, 1, 0.5]))
    tail_block = DirectFormI.from_model(signal.dlti(2, [1, -0.5]))
    x, y = sympy.symbols("x y")

    diagram = series_diagram([head_block, tail_block], input_=x, output=y)

    paths = list(nx.all_simple_edge_paths(diagram, source=x, target=y))
    assert len(paths) == 1
    path = paths[0]
    assert len(path) == 2
    u, v, i = path[0]
...
def test_parallel_diagram_construction():
    left_block = DirectFormI.from_model(signal.dlti(1, [1, 1, 0.5]))
    right_block = DirectFormI.from_model(signal.dlti(2, [1, -0.5]))
    x, y = sympy.symbols("x y")

    diagram = parallel_diagram([left_block, right_block], input_=x, output=y)
