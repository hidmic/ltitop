...
class Interval:
    ...
    def __abs__(self):
        ...
        upper_bound = np.max(
            [np.abs(self.lower_bound), np.abs(self.upper_bound)], axis=0
        )
    ...
    def __mul__(self, other):
        if isinstance(other, Interval):
            ...
            return Interval(np.min([a, b, c, d], axis=0), np.max([a, b, c, d], axis=0))
    ...
    def __eq__(self, other):
        if isinstance(other, Interval):
            return bool(
                np.all(self.lower_bound == other.lower_bound)
                and np.all(self.upper_bound == other.upper_bound)
            )
        return bool(
            np.all(self.lower_bound == other) and np.all(self.upper_bound == other)
        )
    ...
    def __contains__(self, other):
        if isinstance(other, Interval):
            return bool(
                np.all(self.lower_bound <= other.lower_bound)
                and np.all(other.upper_bound <= self.upper_bound)
            )
        return bool(
            np.all(self.lower_bound <= other) and np.all(other <= self.upper_bound)
        )
