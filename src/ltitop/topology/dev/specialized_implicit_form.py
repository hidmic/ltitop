...
class SpecializedImplicitModel(Model):
    ...
    def _make_algorithm(n_t, n_x, n_u, n_y):
        ...
        subalgorithms.append(
            tuple(
                [
                    Assignment(
                        mT_kk[0], nonassociative(mM[0, :] * mX_k + mN[0, :] * mU_k)
                    )
                ]
                + [
                    Assignment(
                        mT_kk[i],
                        nonassociative(
                            -mJ[i, :i] * mT_kk[:i] + mM[i, :] * mX_k + mN[i, :] * mU_k
                        ),
                    )
        ...
        subalgorithms.append(
            tuple(
                Assignment(
                    mX_kk[i],
                    nonassociative(
                        mK[i, :] * mT_kk + mP[i, :] * mX_k + mQ[i, :] * mU_k
                    ),
                )
                for i in range(n_x)
            )
        ...
        subalgorithms.append(
            tuple(
                Assignment(
                    mY_k[i],
                    nonassociative(
                        mL[i, :] * mT_kk + mR[i, :] * mX_k + mS[i, :] * mU_k
                    ),
                )
                for i in range(n_y)
            )
    ...
    def __new__(cls, J, M, N, K, P, Q, L, R, S):
        parameters = cls.Parameters(J, M, N, K, P, Q, L, R, S)
        algorithm = cls._make_algorithm(
            parameters.n_t, parameters.n_x, parameters.n_u, parameters.n_y
        )
    ...
    def process(self, U, X=None):
        ...
        return self.algorithm.perform(X[k], U[k], J, M, N, K, P, Q, L, R, S)
