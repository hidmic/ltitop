...
class DirectForm(Realization):
    ...
    def _make_algorithm(cls, *, n_b, n_a, has_k):
        ...
        if n_a == 0:
            raise ValueError("no a[n] coefficients")
        return cls._make_direct_algorithm(n_b=n_b, n_a=n_a, has_k=has_k)
    ...
    def from_model(cls, model, dtype=None, **kwargs):
        ...
        if dtype is not None:
            ...
            if np.any(mask):
                # Scale coefficients
                k = max(
                    int(math.ceil(math.log2(np.max(np.abs(coefficients[mask]))))), 1
                )
                coefficients = np.ldexp(coefficients, -k)
            coefficients = np.array([dtype(coeff) for coeff in coefficients])
            ...
            if b.size == 0:
                ...
                a = np.concatenate((a[0:1], np.trim_zeros(coefficients[len(b) :], "b")))
        if k is not None:
    ...
    def _pretty(self, printer):
        ...
        for i, c in enumerate(self.parameters.b):
            if c:
                term = printer._print(np.ldexp(float(c), k) * z ** -i)
...
class DirectFormII(DirectForm):
    ...
    def _make_direct_algorithm(cls, *, n_b, n_a, has_k):
        ...
        if n_s > 0:
            ...
            procedure = [
                ...
                Assignment(
                    lhs=s[n],
                    rhs=(x[n] - f(sympy.Sum(a[i] * s[n - i], (i, 1, n_a - 1)))).doit(),
                ),
                Assignment(
                    lhs=y[n], rhs=f(sympy.Sum(b[i] * s[n - i], (i, 0, n_b - 1))).doit()
                ),
    ...
    def _make_stateful_computation_error_model(self):
        ...
        if n_s > 1:
            ...
            A[0, :] = -a[1 : n_s + 1]
            A[1:, :-1] = np.eye(n_s - 1)
            C = np.array([-b[0] * a[i] + b[i] for i in range(1, n_s + 1)])
        else:
            A = -a[1]
            C = -b[0] * a[1] + b[1]
...
class TransposedDirectFormII(DirectForm):
    ...
    def _make_direct_algorithm(cls, *, n_b, n_a, has_k):
        ...
        if n_s > 0:
            ...
            procedure = [Assignment(lhs=tuple(s[i][n] for i in range(n_s)), rhs=sv)]
            procedure.append(Assignment(lhs=y[n], rhs=f(b[0] * x[n] + s[0][n])))
            procedure.extend(
                Assignment(
                    lhs=s[i][n - 1],
                    rhs=(b[i + 1] * x[n] - a[i + 1] * y[n] + s[i + 1][n]),
                )
                for i in range(n_s - 1)
            )
            procedure.append(
                Assignment(lhs=s[n_s - 1][n - 1], rhs=(b[n_s] * x[n] - a[n_s] * y[n]))
            )
            procedure.append(
                Assignment(lhs=sv, rhs=tuple(s[i][n - 1] for i in range(n_s)))
            )
            states = (sv,)
        else:
            procedure = Assignment(lhs=y[n], rhs=f(b[0] * x[n]))
