...
from ltitop.common.arrays import simple_if_possible, split, vectorize
...
def partial_fractions_expansion(z, p, tol=1e-16, rtype="avg"):
    ...
    for j, (vj, sj) in enumerate(zip(v, sv)):
        ...
        def Dj(x):
            return np.prod((x - vm) ** sv)

        r[n] = N(vj) / Dj(vj)
        if sj > 1:

            def h(r):
                return np.sum(su / (vj - u) ** r) - np.sum(sv / (vj - vm) ** r)
...
def summation_decomposition(z, p, k, nterms=2, variant=0, tol=1e-10, rtype="avg"):
    ...
    if len(z) >= len(p):
        ...
        direct_terms, r = poly.divide(poly.from_roots(z), poly.from_roots(p))
        z = np.roots(r)
    ...
    while indices:
        ...
        if not np.isreal(p[j]):
            if not indices:
                raise ValueError(f"No complex conjugate pole for {p[j]}")
            pj_star = np.conj(p[j])
            pp = p[indices]
            ss = s[indices]
    ...
    for indices in indices_per_term:
        ...
        nth_term_gain = k if np.any(nth_term_residues) else 0.0
        terms.append((nth_term_zeros, nth_term_poles, nth_term_gain))
    if direct_terms:  # append them to the first term
        z, p, k = terms[0]
        z = np.roots(
            poly.add(
                poly.multiply(poly.from_roots(p), direct_terms), poly.from_roots(z)
            )
        )
        terms[0] = z, p, k
    return terms
...
def real_product_decomposition(zeros, poles, gain, nterms=2, variant=0, tol=1e-16):
    ...
    pole_groups_per_term = split(
        pole_groups, sorted(r.sample(range(1, len(pole_groups)), nterms - 1))
    )
    ...
    if len(zeros) <= len(poles):  # preserve property
        ...
        zero_groups_per_term = split(
            zero_groups, sorted(r.sample(range(1, len(zero_groups)), nterms - 1))
        )
...
def complex_taylor_series_expansion(f, z, s, dx=1e-6):
    ...
    for i in range(1, s_max):
        ...
        f.append(
            functools.partial(scipy.misc.derivative, f[0], dx=dx, n=i, order=2 * i + 1)
        )
    F = np.full((len(z), s_max), fill_value=np.nan, dtype=complex)
...
def hermite_companion_pencil(f, alpha, z, s):
    ...
    d = np.sum(s) - 1
    C0 = np.zeros((d + 2, d + 2), dtype=complex)
    C0[0, 1:] = np.concatenate([alpha[i, :si] for i, si in enumerate(s)])
    C0[0, 1:] = C0[0, 1:] / np.max(np.abs(C0[0, 1:]))
    C0[1:, 0] = np.concatenate([f[i, :si] for i, si in enumerate(s)])
...
def newtonian_interpolant_denominator(f, z, s, m, k):
    ...
    if k > 0:
        ...
        for j in range(1, m + 1):
            for i in range(m + 1 - j):
                if t[i] != t[i + j]:
                    ddiff_table[i, j] = (
                        ddiff_table[i + 1, j - 1] - ddiff_table[i, j - 1]
                    ) / (t[i + j] - t[i])
                else:
        ...
        for i in range(1, k + 1):
            ...
            for j in range(1, m + 2):
                if t[m + 1 - j] != t[m + i]:
                    ddiff_table[m + 1 - j, j] = (
                        ddiff_table[m + 2 - j, j - 1] - ddiff_table[m + 1 - j, j - 1]
                    ) / (t[m + i] - t[m + 1 - j])
                else:
        ...
        (indices,) = np.where(np.diagonal(U) == 0.0)
    ...
    v[d] = 1.0
    base = 0
    beta = np.full((len(z), np.max(s)), fill_value=np.nan, dtype=complex)
...
def newtonian_to_lagrange_hermite(beta, z, s):
    ...
    """
    n = len(z) - 1
    alpha = np.full((n + 1, np.max(s)), np.nan, dtype=complex)
    beta = np.array(beta)
    for k in range(n):
...
def hermite_rational_interpolant(f, z, s, m, k):
    if m < k:
        raise ValueError(
            f"Prescribed numerator degree ({m}) is"
    ...
    gain = poly.summation(
        alpha[i, p] * f[i, j] * poly.from_roots(z, s + kd[i] * (j - p - 1))
        for i in range(n + 1)
        for p in range(s[i])
        for j in range(p + 1)
    )[n - m]
...
def real_summation_partition(zpk, variant=0, tol=1e-16):
    ...
    if len(pole_groups) < 2:
        return zpk, ((), (), 0.0)  # degenerate case
    r.shuffle(pole_groups)
    ...
    def f(z):
        return -(
            reduce(auto.numpy.multiply, z - q, 1.0)
            / reduce(auto.numpy.multiply, z - u, 1.0)
        )
